...
function foods(meal_name){
  return database.raw(`SELECT f.id food_id, f.name food_name, f.calories food_calories FROM meals m
                      JOIN mealFoods ON m.id = mealFoods.meal_id
                      JOIN foods f ON f.id = mealFoods.food_id
                      WHERE m.name = ?`, meal_name)
}
